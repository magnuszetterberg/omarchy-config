...
Panel {
  id: root
  moduleName: "magnus.power"
  // The shell's PATH puts /usr/share/omarchy/bin first, so the override
  // scripts (which add the "Are u nuts?" profile) are called by absolute path.
  ...
  function setProfile(profile) {
    ...
    actionProc.command = ["/home/magnus/.local/share/omarchy-overrides/bin/omarchy-powerprofiles-set", root.discharging ? "battery" : "ac", profile]
  ...
  Process {
    id: profilesProc
    command: ["/home/magnus/.local/share/omarchy-overrides/bin/omarchy-powerprofiles-list", "--active-state"]
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      onMoveRequested: function(dx, dy) {
        ...
        else if (dy !== 0) root.selectProfileByDelta(dy * profileRow.columns)
      }
      ...
      Column {
        ...
        Column {
          ...
          // Two columns so a fourth profile ("Are u nuts?", raised CPU power
          // caps, added by the omarchy-powerprofiles-* overrides) fits without
          // the labels overflowing their buttons.
          Grid {
            id: profileRow
            width: parent.width
            columns: 2
