...
Item {
  ...
  // --map` (backed by a cache file, so re-requesting on every open is cheap
  // and picks up keybinding changes).
  property var shortcutHints: ({})
  onOpenedChanged: {
    if (!opened) { deleteConfirmOpen = false; deleteTarget = null }
    else refreshShortcutHints()
  }
  function refreshShortcutHints() {
    if (shortcutHintProc.running) return
    shortcutHintProc.collected = ""
    shortcutHintProc.running = true
  }
  ...
  readonly property var providers: ({
    ...
    "power-profiles": {
      script: "current=$(/home/magnus/.local/share/omarchy-overrides/bin/omarchy-powerprofiles-list --active-state 2>/dev/null | awk -F'\\t' '$2 == 1 { print $1 }'); /home/magnus/.local/share/omarchy-overrides/bin/omarchy-powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "/home/magnus/.local/share/omarchy-overrides/bin/omarchy-powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  Process {
    id: shortcutHintProc
    command: ["omarchy-launch-hint", "--map"]
    property string collected: ""
    stdout: SplitParser {
      onRead: function(data) { shortcutHintProc.collected += data }
    }
    onExited: function(exitCode) {
      if (exitCode !== 0) return
      try { root.shortcutHints = JSON.parse(shortcutHintProc.collected || "{}") } catch (e) {}
    }
  }
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              Column {
                ...
                anchors.right: shortcutHintText.left
              ...
              Text {
                id: shortcutHintText
                text: row.isApp ? (root.shortcutHints[row.appId] || "") : ""
